...
class ProductDetailCubit extends Cubit<ProductDetailState> {
  ...
  void addToCart(AddToCartEntity body) async {
    emit(state.copyWith(addToCartState: ViewData.loading(message: 'Loading')));

    final result = await addToCartUseCase.call(body);
    return result.fold(
      (failure) => _onFailureAddToCart(failure),
      (data) => _onSuccessAddToCart(body),
    );
  }

  Future<void> _onFailureAddToCart(
    FailureResponse failure,
  ) async {
    emit(state.copyWith(
        addToCartState:
  ...
  Future<void> _onSuccessAddToCart(
